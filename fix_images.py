...
def normalize_image(image, dim: int = 256):
    """
    Normaliza a imagem
...
def conv_unet(layer, filters=32, kernel=(3, 3), act="relu", i=1, j=1):
    # Define os nomes das layers
    conv_name = "Conv{}_{}".format(i, j)
    bn_name = "BN{}_{}".format(i, j)
    act_name = "Act{}_{}".format(i, j)

    layer = Conv2D(filters=filters, kernel_size=kernel, padding="same", name=conv_name)(
        layer
    )
...
def model_unet(
    ...
    for i in range(depth):
        ...
        for j in range(2):
            layer = conv_unet(first_layer, filters, (3, 3), activation, i, j)
        ...
        if i < depth - 1:
            ...
            first_layer = MaxPooling2D((2, 2), padding="same", name=max_name)(layer)
    ...
    for i in range(depth - 2, -1, -1):
        ...
        for j in range(2, 4):
            layer = conv_unet(layer, filters, (3, 3), activation, i, j)
...
for _type in old_data:
    ...
    for _class in os.listdir(type_path):
        ...
        for image_path in os.listdir(class_path):
            ...
            # ax3.axis('off')
            # plt.show()
            new_path = "{}/{}/{}/{:04d}.png".format(new_data, _type, _class, id)
