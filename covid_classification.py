...
params = {'dim': DIM_SPLIT, 'batch_size': BATCH_SIZE, 'n_class': len(labels), 'shuffle': True, 'channels': 3}
...
for model, net_path in zip(NETS, nets_path):
    ...
    model_params = {'labels': labels, 'model': model, 'model_input_shape': (DIM_SPLIT, DIM_SPLIT, 3)}
    ...
    if weight is not None:
        ...
        fit_params = {'epochs': EPOCHS, 'shuffle': True, 'workers': 1, 'batch_size': BATCH_SIZE}
        history = covid.fit_generator(train_generator=train_generator, val_generator=None, **fit_params)
        weight = covid.save(path=net_path, model=model, history=history.history, metric='accuracy')
